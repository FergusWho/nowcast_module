...
with open('transport_earth/output.json', 'r') as read_file:
...
for ii in range(0, shell_num):
       ...
       mars_r, mars_lat, mars_lon = find_location('planets/mars', current_time, root_dir1)
       venus_r, venus_lat, venus_lon = find_location('planets/venus', current_time, root_dir1)

       phi_STA = phi_e + STA_lon - earth_lon
       phi_mars = phi_e + mars_lon - earth_lon
       phi_venus = phi_e + venus_lon - earth_lon
       ...
       if current_time > datetime.strptime('2018-249', "%Y-%j"):
              psp_r, psp_lat, psp_lon = find_location('spacecraft/psp', current_time, root_dir1)
              ...
              mid_pos = [[1.0, phi_e], [mars_r, phi_mars],[STA_r, phi_STA],[venus_r, phi_venus],[psp_r, phi_psp]]
       else:
              mid_pos = [[1.0, phi_e], [mars_r, phi_mars],[STA_r, phi_STA],[venus_r, phi_venus]]
       ...
       ax2.plot(target[3][0], target[3][1], 'o', linewidth=1.5, ms=12, mec='k', mfc = 'g', label = 'Venus')
       ax2.plot(target[2][0], target[2][1], 's', linewidth=1.5, ms=12, mec='k', mfc = 'r', label = 'STEREO-A')

       if current_time > datetime.strptime('2018-249', "%Y-%j"):
              ax2.plot(target[4][0], target[4][1], 's', linewidth=1.5, ms=12, mec='k', mfc = 'g', label = 'PSP')

       ax2.legend(fontsize=15, loc='lower right', bbox_to_anchor=(1.07, -0.17))
       ...
       # adding text
       plt.figtext(0.51,0.07, 'Trigger: '+trigger, fontsize=20)
       plt.figtext(0.51,0.03, 'dt ='+shell_time[ii]+'    t='+shell_time_str[ii], fontsize=20)
